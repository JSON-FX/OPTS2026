...
- [x] Transactions automatically assigned active workflow on creation
- [x] current_step_id set to first workflow step
- [x] current_office_id set to creating user's office
- [x] current_user_id set to creating user
- [x] received_at set on creation
- [x] Creation fails gracefully when no active workflow
- [ ] Creation fails gracefully when user has no office (deferred - not blocking)
- [x] Transaction create forms show workflow preview
- [x] No-workflow warning displayed when applicable
- [ ] "Create & Endorse" option works correctly (deferred to future story)
- [ ] Immediate endorsement transitions status to "In Progress" (deferred to future story)
...
**Assigned**: Claude Code Team (backend-dev, frontend-dev)
**Start Date**: 2026-02-09
**Completion Date**: 2026-02-09

### Files Created
- [x] `app/Services/WorkflowAssignmentService.php`
- [x] `app/Exceptions/NoActiveWorkflowException.php`
- [x] `resources/js/Components/WorkflowPreviewCard.tsx`
- [x] `tests/Feature/WorkflowAssignmentTest.php`
- [x] `tests/Unit/Services/WorkflowAssignmentServiceTest.php`
- [x] `tests/e2e/workflow-assignment.spec.ts`

### Files Modified
- [x] `app/Http/Controllers/PurchaseRequestController.php`
- [x] `app/Http/Controllers/PurchaseOrderController.php`
- [x] `app/Http/Controllers/VoucherController.php`
- [x] `resources/js/Pages/PurchaseRequests/Create.tsx`
- [x] `resources/js/Pages/PurchaseOrders/Create.tsx`
- [x] `resources/js/Pages/Vouchers/Create.tsx`

### Test Results
- [x] Feature tests: 7 passing (28 assertions)
- [x] Unit tests: 13 passing (54 assertions)
- [x] Total: 20 passing (82 assertions)
- [x] E2E tests: 10 Playwright test cases

### Implementation Notes
- WorkflowAssignmentService provides getActiveWorkflow(), hasActiveWorkflow(), assignWorkflow(), and getWorkflowPreview() methods
- Workflow assignment is optional - if no active workflow exists for a category, transaction creation continues without assignment (wrapped in try-catch for NoActiveWorkflowException)
- assignWorkflow() sets workflow_id, current_step_id (first step), current_office_id (user's office), current_user_id (user), received_at (now)
- WorkflowPreviewCard is a reusable React component showing workflow name, step count, total days, visual route, and expandable step details
- PO and VCH create pages updated from disabled placeholder dropdowns to functional workflow selects (PR already had this)
- "Create & Endorse" immediate endorsement option deferred (not critical for initial workflow assignment)
